...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   elements_check_bonus.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: yoayedde <yoayedde@student.42.fr>          #+#  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025-01-23 01:15:31 by yoayedde          #+#    #+#             */
/*   Updated: 2025-01-23 01:15:31 by yoayedde         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_valid(t_map *map, int x, int y)
{
	...
	if (y < 0 || !map->map[x][y])
		return (0);
	if (map->map[x][y] == '1' || map->map[x][y] == 'N')
		return (0);
	return (1);
}
	...
	while ((*map)->map[++i])
	{
		...
		while ((*map)->map[i][++len])
		{
			if ((*map)->map[i][len] == 'C')
				...
				(*map)->exit += 1;
			else if ((*map)->map[i][len] != '1' && (*map)->map[i][len]
					!= '0' && (*map)->map[i][len] != 'N')
				return (0);
		}
	}
